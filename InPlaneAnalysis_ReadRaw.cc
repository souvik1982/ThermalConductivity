...
int main(int argc, char **argv)
{
  ...
  cout<<"Length (um): "<<meta_length<<" +/- "<<meta_lengthErr<<endl;
  cout<<"Width (um): "<<meta_width<<" +/- "<<meta_widthErr<<endl;
  cout<<"Thickness (um): "<<meta_thickness<<" +/- "<<meta_thicknessErr<<endl;
  ...
  for (unsigned int i = 0; i < n_heaterThermistors; ++i)
  {
    ...
    {
      if (v_heaterChannel.at(i) + "_inplane" == v_label.at(j).substr(2, 11))
      {
  ...
  for (unsigned int i = 0; i < n_coolerThermistors; ++i)
  {
    for (unsigned int j = 1; j < v_label.size(); ++j)
    {
      if (v_coolerChannel.at(i) + "_inplane" == v_label.at(j).substr(2, 11))
      {
  ...
  ofs_datacard<<"In-plane thermal conductivity measurement"<<endl;
